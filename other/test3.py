...
dkdata1=DraftKings1()
dkdata1.get_data(odds=True)
# Example usage with the odds data
odds_data = dkdata1.odds_data_json
df_odds = pd.json_normalize(odds_data, record_path=['eventGroup', 'offerCategories', 'offerSubcategoryDescriptors', 'offerSubcategory', 'offers'], meta=[['eventGroup', 'eventGroupId'], ['eventGroup', 'name']], errors='ignore')

for category in odds_data['eventGroup']['offerCategories']:
    for descriptor in category['offerSubcategoryDescriptors']:
        if descriptor.get('offerSubcategory'):
            for offers in descriptor['offerSubcategory']['offers']:
                for offer in offers:
                    for outcome in offer['outcomes']:
                        if outcome['participant'] in teams_of_interest:
